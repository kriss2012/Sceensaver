# CMatrix Video Screensaver Automatic Installer
# Run this script in PowerShell to automatically set up the screensaver.

# Get the absolute path to CMatrixScreensaver.scr in this directory
$scrPath = Join-Path -Path $PSScriptRoot -ChildPath "CMatrixScreensaver.scr"

# Ensure the screensaver file exists
if (Test-Path $scrPath) {
    Write-Host "Configuring Windows Screensaver registry settings for CMatrix..."
    ...
    Write-Host ""
    Write-Host "SUCCESS! CMatrix Video Screensaver has been configured and activated."
    Write-Host "Path: $scrPath"
    Write-Host "Idle Timeout: 1 minute (60 seconds)"
    Write-Host "Note: You can test the screensaver immediately by running: .\CMatrixScreensaver.scr /s"
} else {
    Write-Error "Error: CMatrixScreensaver.scr was not found in: $PSScriptRoot. Please run compilation first."
}
